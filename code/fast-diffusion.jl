##
using MatrixNetworks, SparseArrays, LinearAlgebra, Distributions, DelimitedFiles, ProgressMeter, Random, Distributed
# using StatsBase
##
#=
...
mutable struct SIRData
  ...
  log1beta::Float64
  log1gamma::Float64
  inNeighborsA::Vector{Vector{Int}} #adjlist representation of in-neighbors of A
  outNeighborsA::Vector{Vector{Int}} #adjlist representation of out-neighbors of A
...
mutable struct SEIRData
  ...
  log1beta::Float64
  log1gamma::Float64
  inNeighborsA::Vector{Vector{Int}} #adjlist representation of A
  outNeighborsA::Vector{Vector{Int}}
  itime::Vector{Int} # infection time
  ctime::Vector{Int} # contagion time
  rtime::Vector{Int} # recovery time
  snodes::Vector{Bool} # still suceptible nodes
...
function _clear!(E::Union{SIRData,SEIRData})
  """
    resetting the data structure E for new diffusion
  """
...
function getneighbors(A::SparseMatrixCSC)
  """
      ...
      assumes A[i,j]=1 ⟺ i→j is a directed edge
  """
...
function SIRData(A::SparseMatrixCSC;beta::Float64=0.1,gamma::Float64=0.05,tmax::Int=3500,qcap::Int=0)
  ...
  ihist = getneighbors(A) # this is somehow faster than a deepcopy
...
function _quarantine_node(E::Union{SIRData,SEIRData},t::Int,n::Int,qtime::Int,tmax::Int,neighbors::Bool=true;tdelay::Int=1)
  ...
  if neighbors #we can still try to quarantine neighbors even if n is already quarantined or if there isnt space now
    if tdelay==0
      for v in E.outNeighborsA[n] # quarantine
        E.qtimes[v] = t
        _quarantine_node(E,t,v,qtime,tmax,false)
      end
    else
      @inbounds for v in E.outNeighborsA[n] # schedule quarantine
...
function _adjust_itimes(E::Union{SIRData,SEIRData},t::Int,n::Int,qtime::Int,tmax::Int)
  ...
  if !E.snodes[n] #already scheduled stuff so need to adjust itimes of outneighbors
    @inbounds for v in E.outNeighborsA[n]
...
function _post_quarantine_infection(E::Union{SIRData,SEIRData}, t::Int, n::Int)
  ...
  """
  for (ind,v) in enumerate(E.inNeighborsA[n]) #nodes that point to n
    ...
    if q #quarantine 
      if ninfs>100 #inf detectablity threshold in population
        if E.qcurr[t]<E.qcap #space to quarantine
          ...
          @inbounds for c = 1:nnodes #TODO: current bottleneck. need to avoid iterating over all nodes 
...
function partial_epidemic(E::Union{SEIRData,SIRData}, seed::Int, target_infs::Int;
  tmax::Int = 3500, q::Bool = false, rseed=-1, tdelay=1, kwargs...)
  """
    main epidemic function. At each time step, dequeue from event times and using
    node state, figure out what action is happening for each node.
  ...
  nnodes = length(E.snodes)
  ninfs = nnodes-sum(E.snodes) #total infs
  @inbounds while t <= tmax && ninfs < target_infs
    # if length(Set(E.etimes.buckets[t]))<length(E.etimes.buckets[t])
    #   println("a node appears $(maximum(counts(E.etimes.buckets[t]))) times at time t=$t")
    # end
    while !isempty(E.etimes) && peek(E.etimes)[2] <= t
...
function get_infdata(lastactive::Int,E::Union{SIRData,SEIRData};method::String="ctime")
  """
    returns vectors of net new infs and new infs at each time step. uses contagious time
  """
  infs=zeros(Int,lastactive)
  recovs=zeros(Int,lastactive)
  if method=="itime"
    @inbounds for node=1:length(E.itime)
      ct,rt = E.itime[node],E.rtime[node]
      if ct<lastactive
  ...
  elseif method=="ctime"
    ...
    ErrorException("method for counting infections must be one of itime or ctime")
  end
end
...
function qdiffusion1(nnodes::Int,
  ...
  qpercents::Vector{T}=collect(0:15),
  ...
  node_rseed::Int=71) where T<:Real
  ...
  @inbounds for (i,g) in enumerate(gnames)
    ...
    if g != gnames[1] #dont reload first graph
      # @everywhere A = loadGraph(g,gpath,sym=true)
      @everywhere @eval A = loadGraph($g,$gpath,sym=true)
    end
    #node sample
    ...
    #modify beta depending on what we wish to preserve from the base network
    #however we will keep the beta from the original network. this will make loading 
    #in files much easier but may be misleading. will add a flag to the filename to note
    #this. This is only for certain experiments. 
    # if canonical_graph_name(g)!=g #not the base network
    #   #measure thingy from current network

    #   #compare to base network and update values of betas 
    # end
    ...
    for (i,b) in enumerate(betas)
      ...
      #modify parameters based on what we wish to preserve.. seems adhoc.
...
## r0 estimation fcns
function allmins(data::Vector{T} where T<:Union{Int,Float64,Float32})
  """
    mininds = (all indicies in data that correspond to the minimum value in data)
    minval = minimum(data)
  """
  mininds = ones(Int,1)
  minval = data[1]
  for i=2:length(data)
    if data[i]<minval
      minval = data[i]
      mininds = fill!(ones(Int,1),i)
    elseif data[i]==minval
      push!(mininds,i)
    end
  end
  return mininds,minval
end
# this fcn was broken but i left it just in case
# function secondary_infs(E::Union{EventEpidemic.SIRData,EventEpidemic.SEIRData},seed::Int;
#           ktrials::Int=1,tmax::Int=3500,tdelay::Int=1,inflb::Int=0)
#   """run full diffusion and return number of primary and secondary infs"""
#   l,E = EventEpidemic.epidemic(E,seed,tmax=tmax,q=false,tdelay=tdelay)
#   netinfs,newinfs = EventEpidemic.get_infdata(l,E)
#   while sum(newinfs)<inflb #must infect at least one node
#     l,E = EventEpidemic.epidemic(E,seed,tmax=tmax,q=false,tdelay=tdelay)
#     netinfs,newinfs = EventEpidemic.get_infdata(l,E)
#   end
#   #counting secondary infs
#   indshist = first.(allmins.(E.ihist))
#   hist = map(x->E.inNeighborsA[x][indshist[x]],1:length(E.snodes))
#   # primaryinfs = first.(hist).==seed
#   primaryinfs = all.(zip(first.(hist).==seed,(!).(E.snodes))) #inf by seed and actually got infected
#   return count(map(x->any(primaryinfs[hist[x]]),1:length(E.snodes))),count(primaryinfs),sum(newinfs)
# end
function secondary_infs(E::Union{EventEpidemic.SIRData,EventEpidemic.SEIRData},seed::Int;
          ktrials::Int=1,tmax::Int=3500,tdelay::Int=1,inflb::Int=0)
  ...
  indshist = first.(allmins.(E.ihist))
  hist = map(x->E.inNeighborsA[x][indshist[x]],1:length(E.snodes))
  # primaryinfs = fill!(false,Vector{Bool}(undef,length(E.snodes))) #for testing

  c = 0
  @inbounds for v=1:size(A,1)
    if !E.snodes[v] #actually got infected so hist is relevant
      if seed in hist[v]
        # primaryinfs[v] = true
        c+=1
      end
  ...
  return c,sum(newinfs)
end

# assumes that E is loaded in memory
function r0_estimation(v::Int,beta::Float64,gamma::Float64;
      ktrials::Int=1,tmax::Int=3500,tdelay::Int=1,inflb::Int=0)
  update_E!(E,beta,gamma,0)#qcap = 0 for r0 estimation
  return map(x->secondary_infs(E,v,tmax=tmax,tdelay=tdelay,inflb=inflb),1:ktrials)
end
r0_estimation(v::Int;kwargs...) = r0_estimation(v,E.beta,E.gamma;kwargs...)

function distributed_r0_estimation(nnodes::Int,A::SparseMatrixCSC,betas::Vector{Float64},gammas::Vector{Float64};
              ktrials::Int=1,tdelay::Int=1,tmax::Int=3500,inflb::Int=0,sampleseed::Int=71,method::String="sir")
  """distributed primary and secondary infection counts"""
  Random.seed!(sampleseed)
  vs = Distributions.sample(1:size(A,1),nnodes,replace=false)
  #loading relevant parameters into memory on all processors
  @everywhere @eval A=$A
  @everywhere @eval tmax,tdelay,ktrials,vs,inflb = $tmax,$tdelay,$ktrials,$vs,$inflb
  if lowercase(method)=="sir"
    @everywhere E = Main.EventEpidemic.SIRData(A,tmax=tmax,qcap=0)
  elseif lowercase(method)=="seir"
    @everywhere E = Main.EventEpidemic.SEIRData(A,a=a,tmax=tmax,qcap=0)
  else
    @error("must have method=\"sir\" or method=\"seir\"")
  end

  @everywhere tmpfcn((v,beta,gamma,ktrials,tmax,tdelay,inflb)) = r0_estimation(v,beta,gamma;ktrials=ktrials,tmax=tmax,tdelay=tdelay,inflb=inflb)

  ps = vcat(Base.Iterators.product(vs,betas,gammas,ktrials,tmax,tdelay,inflb)...)
  return @showprogress pmap(x->tmpfcn(x),ps)
end#TODO modify the parameters so that ktrials is first then vs then bs then gammas so that we can group and save these better

function get_r0_estimation(gname::String,nnodes::Int,betas::Vector{Float64},gammas::Vector{Float64};
                    ktrials::Int=1,tmax::Int=3500,tdelay::Int=1,inflb::Int=0,sampleseed::Int=71,
                    method::String="sir",gpath::String="",dst::String="")

  #note that if we are conditioning on infecting inflb>0 other nodes, then we are bounding
  #r0 below by inflb-1. so we need to have inflb=1 and dont condition on infecting other nodes
  #load graph, get primary/secondary infs, and save data
  A = loadGraph(gname,gpath,sym=true)
  res = distributed_r0_estimation(nnodes,A,betas,gammas;ktrials=ktrials,tmax=tmax,tdelay=tdelay,method=method,inflb=inflb)
  open(dst*"$(gname[1:end-5])-$(uppercase(method))-$nnodes-$ktrials-$inflb.txt","w") do io
    writedlm(io,vcat(res...),',')
  end
end

function r0_distributed(gname::String,nnodes::Int,betas::Vector{Float64},gammas::Vector{Float64};
                    a::Float64=5.0,tmax::Int=3500,tdelay::Int=1,inflb::Int=0,sampleseed::Int=71,
                    method::String="sir",ktrials::Int=1,nsamples::Int=10,
                    gpath::String="",dst::String="")
  """ for multiple trials """
  A = loadGraph(gname,gpath,sym=true)
  ...
  Random.seed!(time_ns())#randomize diffusions

  #loading relevant parameters into memory on all processors
  @everywhere @eval A=$A
  @everywhere @eval tmax,tdelay,ktrials,vs,inflb,a = $tmax,$tdelay,$ktrials,$vs,$inflb,$a
  if lowercase(method)=="sir"
    @everywhere E = Main.EventEpidemic.SIRData(A,tmax=tmax,qcap=0)
  elseif lowercase(method)=="seir"
    @everywhere E = Main.EventEpidemic.SEIRData(A,a=a,tmax=tmax,qcap=0)
  ...
  for k=1:nsamples
    println("working on sample $k of $nsamples")
    res = @showprogress pmap(x->tmpfcn(x),ps)
    if nsamples==1
      fname = dst*"$(gname[1:end-5])-$(uppercase(method))-$nnodes-$ktrials-$inflb.txt"
    else
      fname = dst*"$(gname[1:end-5])-$(uppercase(method))-$nnodes-$ktrials-$inflb-$k.txt"
    end
    open(fname,"w") do io
      writedlm(io,vcat(res...),',')
    end
  end
end #TODO allow us to pass in nodes directly as well as graph if someone has it loaded in memory
#end r0 estimation fcns
