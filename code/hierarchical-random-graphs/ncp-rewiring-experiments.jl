...
dstDir =  joinpath(mainDir,"scratch","ncp-rewired-graphs-v2")
...
#parameters 
rewiring_ps = vec([0.0001 0.0005 0.001 0.005 0.01 0.05 0.1 0.25 0.5 1.0]) # excludes 10.0 and 100.0
# rewiring_ps = vec([10.0])
...
function filter_rewired_graphs(gname::String,gdst::String)
    ...
    bool = all(map(x->"ncp-rewired-$(round(x*100,digits=2))-v2-$gname" in graphnames,params[gname]))
    return !bool 
end
...
@everywhere function ncp_rewiring(
    ...
    #preprocess sets. 
    #v2 
    ncp,sets = preprocess_sets!(ncp,sets,A,250)
    ...
    #write to dst 
    fname = joinpath(dstDir,"ncp-rewired-$(round(p*100,digits=2))-v2-$gname")  
    writeSMAT(X,fname)
